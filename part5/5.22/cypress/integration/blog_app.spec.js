  ...
  describe('When logged in', function () {
    ...
    describe('and multiple blogs exists', function () {
      ...
      it('blogs are ordered by most likes', function () {
        // define both blogs
        cy.contains('test blog Sammynilla').parent().as('blog1');
        cy.contains('test blog 2 Sammynilla').parent().as('blog2');
        ...
        cy.get('@blog2')
          .find('button').contains('view')
          .as('toggleDetails2');
        // display details of both blogs
        cy.get('@toggleDetails').click();
        cy.get('@toggleDetails2').click();

        // verify that the first blog grabbed now contains 'test blog 2 Sammynilla'
        cy.get('.blog')
          .eq(0)
          .contains('test blog 2 Sammynilla');
      });
    });
  });
});